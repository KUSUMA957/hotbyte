...
public class AuthServiceImpl implements AuthService {
	...
	public Map<String, String> register(RegisterRequest request) {
		...
		User user = User.builder().name(request.getName()).email(request.getEmail())
				.password(passwordEncoder.encode(request.getPassword()))// .password(request.getPassword()) we will
																		// encrypt later
				.phone(request.getPhone()).address(request.getAddress()).gender(request.getGender())
				.role(request.getRole() != null ? request.getRole() : Role.USER).createdAt(LocalDateTime.now()).build();
		User savedUser = userRepository.save(user);
		// ✅ 3. If role = RESTAURANT → create restaurant
		if (savedUser.getRole() == Role.RESTAURANT) {
			Restaurant restaurant = Restaurant.builder().restaurantName(request.getRestaurantName())
					.location(request.getLocation()).contactNumber(request.getPhone()).user(savedUser)
	...
	public Map<String, Object> login(LoginRequest request) {
		...
		return Map.of("message", "Login successful", "token", token, "role", user.getRole().name(), "name",
				user.getName(), "restaurantName", restaurantName != null ? restaurantName : "");
	}
}
